...
namespace OpenData.Framework.Grid
{
    public class GridModel
    {
        ...
        protected virtual void Initialize(ModelMetadata modelMetadata, IEnumerable dataSource, ViewContext viewContext)
        {
            ...
            {
                var gridAttribute = ((BzwayModelMetadata)modelMetadata).Attributes.OfType<GridAttribute>().FirstOrDefault();
            ...
            PageSizeDifinition = ((BzwayModelMetadata)modelMetadata).Attributes.OfType<PageSizeAttribute>().FirstOrDefault();
        ...
        protected virtual IEnumerable<GridAction> GetActions(ModelMetadata modelMetadata, ViewContext viewContext)
        {
            ...
            if (modelMetadata is BzwayModelMetadata)
            {
                foreach (var item in ((BzwayModelMetadata)modelMetadata).Attributes.OfType<GridActionAttribute>().OrderBy(it => it.Order))
                {
        ...
        protected virtual IEnumerable<GridColumn> GetGridColumns(ModelMetadata propertyMetadata)
        {
            if (ModelMetadata is BzwayModelMetadata)
            {
                foreach (var att in ((BzwayModelMetadata)propertyMetadata).Attributes.OfType<GridColumnAttribute>())
                {
        ...
        protected virtual IEnumerable<GridCommandAttribute> GetGridCommandDefinitions(ModelMetadata modelMetadata)
        {
            if (modelMetadata is BzwayModelMetadata)
            {
                return ((BzwayModelMetadata)modelMetadata).Attributes.OfType<GridCommandAttribute>().OrderBy(it => it.Order).ToArray();
            }
            else
            {
