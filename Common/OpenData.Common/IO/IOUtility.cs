...
namespace OpenData.IO
{
    ...
    public static class IOUtility
    {
        ...
        public static void SaveStringToFile(string filePath, string body)
        {
            ...
            {
                fileStream.WriteString(body, System.Text.Encoding.UTF8);
            }
        }
        #endregion
        ...
        public static void CopyDirectory(string source, string dest, bool @override = true)
        {
            ...
            //{
            //    throw new BzwayException("Could not complete operation since target directory is under source directory.".Localize());
            //}
        ...
        DELETE:
            try
            {
                if (Directory.Exists(physicalPath))
                {
                    try
                    {
                        FileSystem.DeleteDirectory(physicalPath, UIOption.OnlyErrorDialogs, RecycleOption.SendToRecycleBin);
                    }
                    catch (IOException)
                    {
        ...
        /// <returns></returns>
        public static IEnumerable<FileInfo> EnumerateFilesExludeHidden(string path, string searchPattern)
        {
            DirectoryInfo dir = new DirectoryInfo(path);
