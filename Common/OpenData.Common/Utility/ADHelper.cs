...
namespace OpenData.Utility
{
	...
	public class ADHelper
	{
		...
		private static string ADFullPath = ConfigurationManager.AppSettings["ADFullPath"].ToString() ;
		private static string ADUser = ConfigurationManager.AppSettings["ADAdminUser"].ToString();
		private static string ADPassword = ConfigurationManager.AppSettings["ADAdminPassword"].ToString();
		private static string ADServer = ConfigurationManager.AppSettings["ADServer"].ToString();
		...
		public static bool UserExistsByLogin( string login )
		{
			//create an instance of the DirectoryEntry
			DirectoryEntry de = GetDirectoryObject();
		...
		public static DirectoryEntry GetUserByLogin( string login )
		{
			//create an instance of the DirectoryEntry
			DirectoryEntry de = GetDirectoryObject();
			...
			if(results !=null)
			{
				...
				for (int l = 0; l < n ; l++)
				{
					//create a link to the user object sot hat the FirstName, LastName and SUername can be gotten
					 DirectoryEntry deUser= new DirectoryEntry(ADFullPath + "/" +pcoll["member"][l].ToString(),ADUser,ADPassword,AuthenticationTypes.Secure);
...
//			oDE.Invoke("accountDisabled",new Object[]{"true"});
		...
		private static DirectoryEntry GetDirectoryObject()
		{
			...
			oDE = new DirectoryEntry(ADFullPath,ADUser,ADPassword,AuthenticationTypes.Secure);
		...
		private static DirectoryEntry GetDirectoryObject(string DomainReference)
		{
			...
			oDE = new DirectoryEntry(ADFullPath + DomainReference,ADUser,ADPassword,AuthenticationTypes.Secure);
