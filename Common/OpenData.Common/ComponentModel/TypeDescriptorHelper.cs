...
namespace OpenData.ComponentModel
{
    /// <summary>
    /// 
    /// </summary>
    public static class TypeDescriptorHelper
    {
        ...
        {
            //The Bzway.dll can not be loaded in Medium trust level
            //http://Bzway.codeplex.com/workitem/12954
            Discover(AppDomain.CurrentDomain.GetAssemblies()
                .Where(it => !it.IsDynamic)
                .Where(it => !it.FullName.StartsWith("System.") && !it.FullName.Contains("Bzway,"))
                .SelectMany(it =>
                {
