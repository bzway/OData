...
namespace OpenData.Website.Form.Html
{
    ...
    {
        static string Table = @"
@model Bzway.Website.Web.Areas.Contents.Models.SelectableViewModel
@using Bzway.Data.Query
@using Bzway.Data.Models
@using Bzway.Website.Web.Areas.Contents.Controllers
@{{
...
";
        ...
        public string Generate(ISchema schema)
        {
            ...
            foreach (var item in schema.Columns)
            {
                if (item.ShowInGrid)
                {
                    string columnValue = string.Format("@Bzway.Website.Form.Html.HtmlCodeHelper.RenderColumnValue(item.{0})", item.Name);
